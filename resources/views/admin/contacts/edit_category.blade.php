<div class="row">
    <div class="col-sm-12">
        <form method="post" action="{{ route('contacts.update',['id' => $contact->id]) }}" class="ajaxForm">
            {{csrf_field()}}
            ...
            <div class="form-group row">
                ...
                <div class="col-8">
                    <select class="form-control select2" id="category_id" name="category_id">
                        ...
                        <option {{ ($category->id == $contact->category_id) ? 'selected' : '' }} value="{{$category->id}}">{{$category->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
